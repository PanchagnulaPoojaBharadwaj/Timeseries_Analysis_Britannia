...
stock_price=na.omit(stock_data$BRITANNIA.NS$BRITANNIA.NS.Adjusted)
#MSFT_price

#stock_price = BRITANNIA.NS$BRITANNIA.NS.Close # Adjusted Closing Price
...
```{r}
# Augmented Dickey-Fuller (ADF) Test for Stationarity with Britannia Data
# *******************************************************************

adf_test_britannia = adf.test(stock_price);adf_test_britannia
# Inference : BRITANNIA Time-Series is Non-Stationary

```

```{r}
britannia_ds = diff(log(stock_price)); plot(britannia_ds) # Britannia (First)return Difference Time-Series
```

```{r}
britannia_ds=na.omit(britannia_ds)
adf_test_britannia_ds = adf.test(britannia_ds); adf_test_britannia_ds # Inference : Britannia Difference Time-Series is Stationary

```

```{r}
# Ljung-Box Test for Autocorrelation - Britannia Data
# ***********************************************

lb_test_britannia_ds = Box.test(britannia_ds); lb_test_britannia_ds # Inference : Britannia Difference (Stationary) Time-Series is Autocorrelated as NULL is rejected and p-value<0.0151 | NULL: No Auto correlation | Alternate: Auto Correlation
```
...
acf(britannia_ds) # ACF of Britannia Difference (Stationary) Series
pacf(britannia_ds) # PACF of Britannia Difference (Stationary) Series
```
...
```{r}
arma_pq_britannia_ds = auto.arima(britannia_ds); arma_pq_britannia_ds #p-lag=2, q-lag=2
```

```{r}
britannia_ds_fpq = forecast(arma_pq_britannia_ds, h = 500)
plot(britannia_ds_fpq)
```
...
lb_test_arma_pq_britannia_ds = Box.test(arma_pq_britannia_ds$residuals); lb_test_arma_pq_britannia_ds
#p-value>alpha
```
...
britannia_ret_sq = arma_pq_britannia_ds$residuals^2 # Residual Variance (Since Mean Returns is approx. 0)
plot(britannia_ret_sq)
britannia_ret_sq_box_test = Box.test(britannia_ret_sq, lag = 2) # H0: Return Variance Series is Not Serially Correlated
britannia_ret_sq_box_test # Inference : Return Variance Series is Autocorrelated (Has Volatility Clustering)
```

```{r}
# Test for Volatility Clustering or Heteroskedasticity: ARCH Test
britannia_ret_arch_test = ArchTest(arma_pq_britannia_ds$residuals^2, lags = 2) # H0: No ARCH Effects
britannia_ret_arch_test # Inference : Return Series is Heteroskedastic (Has Volatility Clustering)
```
...
britannia_ret_garch1 = ugarchfit(garch_model1, data = arma_pq_britannia_ds$residuals^2); britannia_ret_garch1

```

```{r}
# Test for Volatility Clustering or Heteroskedasticity: ARCH Test
britannia_garch_arch_test = ArchTest(residuals(britannia_ret_garch1)^2, lags = 1) # H0: No ARCH Effects
britannia_garch_arch_test # Inference : Return Series is Heteroskedastic (Has Volatility Clustering)
#britannia_ret_garch1
```
...
britannia_ret_garch2 = ugarchfit(garch_model2, data = britannia_ds); britannia_ret_garch2

# GARCH Forecast
britannia_ret_garch_forecast1 = ugarchforecast(britannia_ret_garch1, n.ahead = 500); britannia_ret_garch_forecast1
britannia_ret_garch_forecast2 = ugarchforecast(britannia_ret_garch2, n.ahead = 500); britannia_ret_garch_forecast2
```

```{r}
plot(britannia_ret_garch_forecast2)
```
